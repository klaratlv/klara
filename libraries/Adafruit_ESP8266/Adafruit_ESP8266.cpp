...
boolean Adafruit_ESP8266::requestURL(char* url, char* method) {
    print(F("AT+CIPSEND="));
    println(22 + strlen(method) + strlen(url) + strlen_P((Pchr *)host));
    if(find(F("> "))) { // Wait for prompt
        print(method);
        print(F(" ")); // 1
        print(url);
...
int Adafruit_ESP8266::available() {
  return stream->available();
}

char Adafruit_ESP8266::read() {
  return stream->read();
}
